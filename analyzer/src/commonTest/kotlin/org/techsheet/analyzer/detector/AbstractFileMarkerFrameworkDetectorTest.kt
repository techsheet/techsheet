...
import org.techsheet.analyzer.detector.AbstractFileMarkerFrameworkDetector
...
class AbstractFileMarkerFrameworkDetectorTest {
  ...
  @Test fun `filename-only marker registers framework with no version`() {
    val result = _root_ide_package_.org.techsheet.analyzer.detector.FilenameOnlyMarker()
      .onMatch("manage.py".toPath(), lazy { null }, DetectionResult())
  ...
  @Test fun `content-gated marker registers when content contains the signal`() {
    val result = _root_ide_package_.org.techsheet.analyzer.detector.ContentMatchNoVersion().onMatch(
  ...
  @Test fun `content-gated marker captures version from group 1`() {
    val versionPhp = "<?php\n\$wp_version = '6.5.2';\n\$wp_db_version = 57155;\n"
    val result = _root_ide_package_.org.techsheet.analyzer.detector.ContentMatchWithVersion().onMatch(
