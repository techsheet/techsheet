...
import org.techsheet.analyzer.detector.AbstractPhpFrameworkDetector
import org.techsheet.analyzer.detector.laravelVersion
...
private fun analyzePhp(
  ...
  content: String,
  packageNames: List<String> = listOf("laravel/framework"),
): DetectionResult = _root_ide_package_.org.techsheet.analyzer.detector.PhpTestDetector(packageNames)
...
class AbstractPhpFrameworkDetectorTest {
  ...
  @Test fun `composer json detects package in require with caret version`() {
    ...
    """.trimIndent()
    assertEquals("11.0", _root_ide_package_.org.techsheet.analyzer.detector.analyzePhp("composer.json", json)
      .laravelVersion())
  }
  ...
  @Test fun `composer json detects package in require-dev`() {
    ...
    """.trimIndent()
    assertEquals("10.5.2", _root_ide_package_.org.techsheet.analyzer.detector.analyzePhp("composer.json", json)
      .laravelVersion())
  }

  @Test fun `composer json strips greater-than-or-equal prefix`() {
    val json = """{"require":{"laravel/framework":">=9.1"}}"""
    assertEquals("9.1", _root_ide_package_.org.techsheet.analyzer.detector.analyzePhp("composer.json", json)
      .laravelVersion())
  }

  @Test fun `composer json missing package leaves result unchanged`() {
    val json = """{"require":{"symfony/console":"^6.0"}}"""
    assertTrue(_root_ide_package_.org.techsheet.analyzer.detector.analyzePhp("composer.json", json).isEmpty())
  }

  @Test fun `composer json with multiple package names matches any one`() {
    val json = """{"require":{"symfony/symfony":"^5.4"}}"""
    val result = _root_ide_package_.org.techsheet.analyzer.detector.analyzePhp(
      "composer.json", json,
  ...
  @Test fun `empty composer json leaves result unchanged`() {
    assertTrue(_root_ide_package_.org.techsheet.analyzer.detector.analyzePhp("composer.json", "").isEmpty())
  }
  ...
  @Test fun `composer lock detects resolved version from packages entry`() {
    ...
    """.trimIndent()
    assertEquals("11.5.2", _root_ide_package_.org.techsheet.analyzer.detector.analyzePhp("composer.lock", lock)
      .laravelVersion())
  }
  ...
  @Test fun `composer lock without target package leaves result unchanged`() {
    ...
    """.trimIndent()
    assertNull(_root_ide_package_.org.techsheet.analyzer.detector.analyzePhp("composer.lock", lock).laravelVersion())
  }
}
