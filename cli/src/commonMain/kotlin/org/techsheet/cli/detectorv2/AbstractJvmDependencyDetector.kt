 ...
 *
 * Version-ref resolution is intra-file: property references inside a single POM or Gradle
 * script are resolved against declarations in the same file. Cross-file resolution (e.g.
...
abstract class AbstractJvmDependencyDetector(
    ...
    POM_BLOCK
      ...
      ?.let { blocks ->
        ...
        blocks.fold(sheet) { acc, block ->
          acc.withFramework(framework, resolvePomVersion(block, props))
        }
      }
      ?: sheet
  ...
  private fun parseGradle(text: String, sheet: TechSheet): TechSheet =
    (coordinates.asSequence().flatMap { coord ->
      ...
      ?.let { hits ->
        ...
        hits.fold(sheet) { acc, raw -> acc.withFramework(framework, resolveGradleVersion(raw, props)) }
      }
      ?: sheet
  ...
  private fun parseCatalog(text: String, sheet: TechSheet): TechSheet =
    (catalogLibraryVersions(text) + catalogPluginVersions(text))
      .toList()
      .takeIf { it.isNotEmpty() }
      ?.fold(sheet) { acc, version -> acc.withFramework(framework, version) }
      ?: sheet
